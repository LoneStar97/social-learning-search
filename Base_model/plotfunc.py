...
matplotlib.use('Agg')  # Non-interactive (using for hpc)
# In[1]:
...
def timedata(targets, beta, agents, mu, alpha, rho, Rv, tick_speed, runs, hdf5filename, CaseCount, target_location, save):
    trees = [targets]
    betas = [beta]
    agents = [agents]
    mu_range = [mu]
    ...
    while z < runs:
        ...
        main_timeana(paramlist[0], tick_speed, hdf5filename, caseid, target_location)
        z += 1
        
def draw_video(targets, beta, agents, mu, alpha, rho, Rv, tick_speed, runs, videofilename, target_locations, save):
    ...
    while z < runs:
        ...
        result = main_video(paramlist[0], fig, ax, tick_speed, videofilename, target_locations)
        caseid = z 
        if save:
            f2.write(result)
        z += 1
