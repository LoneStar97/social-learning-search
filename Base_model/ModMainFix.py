...
grid_num = 10 # number of cells along each axis
cell_size = 1.0/grid_num # size of each cell
thresholdvalues = 1000 * int(cell_size/minstep) # threshold value to generate a negative target
...
class Grid:
    ...
    def __init__(self, radiuss, tc, agents, beta, l, a, rho, Rv, target_locations):
        ...
        ##create targets 
        self.crea_targets(target_locations)
        ...
        # Initialize a 2D array to store the counts of detected no targets
        self.grid_counts = np.zeros((grid_num, grid_num),dtype=int)
        # Initialize a 2D array to store the counts of detected a target
        self.grid_counts_tar = np.zeros((grid_num, grid_num),dtype=int)
        #----------------------------------------------------------------------------------------------------------
    ...
    def rebuild_kdtree(self):
        """Rebuild the KD-tree with updated targets."""
        coord_targets = np.array([[t.x, t.y] for t in targets_list])
    ...
    def update(self):
        ...
        while z < len(agents_list):
            ...
            # Get the grid cell indices for the agent's position
            x_idx = int(agents_list[z].x // cell_size)
            y_idx = int(agents_list[z].y // cell_size)
            # Check if the agent has just collected a target in this time step
            if agents_list[z].target != 0:
                if 0 <= x_idx < grid_num and 0 <= y_idx < grid_num:
                    self.grid_counts_tar[x_idx][y_idx] += 1
            #    agents_list[z].target = 0  # Reset target flag after counting to prevent re-incrementing
            if agents_list[z].numcoltar == 0:
                # Ensure the indices are within the grid bounds
                if 0 <= x_idx < grid_num and 0 <= y_idx < grid_num:
                    # Increment the count for the corresponding grid cell
                    self.grid_counts[x_idx][y_idx] += 1
                # Add negative target if threshold reached
                if (self.grid_counts[x_idx, y_idx] >= thresholdvalues) & (self.grid_counts_tar[x_idx, y_idx] == 0):
                    center_x = (x_idx + 0.5) * cell_size
                    center_y = (y_idx + 0.5) * cell_size
                    # Avoid duplicates
                    if not any(
                        abs(t.x - center_x) < 1e-3 and 
                        abs(t.y - center_y) < 1e-3 and 
                        t.target_type == 'negative'
                        for t in targets_list
                    ):
                        new_num = len(targets_list) + 1
                        targets_list.append(Patch(center_x, center_y, new_num, 'negative'))
                        detected_negative_targets.append([center_x, center_y])
                        self.rebuild_kdtree()  # Update spatial tree
    ...
    def crea_targets(self, target_locations):
        self.size = 1
...
class Agent:
    ...
    def move_towards_negative_target(self, negative_target):
        ...
        if distance > 0:
            step_length = 0.02 * np.exp(-50 * distance)
...
# In[27]:
def main_video(params, fig, ax, tick_speed, video_filename, target_locations):    
#     from Grid import Grid
    ...
    grid = Grid(radiuss, tc, mono, beta, l, a, rho, Rv, target_locations)
    i = 1
    ...
    #                 ax.scatter(ag_x[g], ag_y[g],  marker=(3, 0, ag_theta[g]), c='blue', s = 40)
                plt.pause(0.01)
                plt.title(('percent_found:'+str(pf)+';tick:'+str(i)+';avg_efficiency:'+str(np.format_float_positional(np.mean(ag_eff), precision=3))+';num_negative_tar:'+str(len(detected_negative_targets))), fontsize = 16)
                fig.canvas.draw()
...
def main_timeana(params, tick_speed, hdf5_filename, case_id, target_locations):
    radiuss = params[3]
    tc = params[0]
    mono = params[1]
    beta = params[2]
    ...
    ggtimes = grid.grid_counts # counts of times of agents visiting the area without detecting a target 
    ggtimes_tar = grid.grid_counts_tar # counts of times of agents collecting targets in the grid cell
    ...
    with h5py.File(hdf5_filename, 'a') as hdf5_file:
        ...
        case_group.create_dataset('NvNoTar', data=np.array(ggtimes), compression="gzip", compression_opts=9)
        case_group.create_dataset('NvTar', data=np.array(ggtimes_tar), compression="gzip", compression_opts=9)
        
        # Save detected target counts
        case_group.create_dataset('target_counts', data=np.array(all_detected_targets), compression="gzip", compression_opts=9)
