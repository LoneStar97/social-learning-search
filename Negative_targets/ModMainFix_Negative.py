# -*- coding: utf-8 -*-
"""
Created on Fri Oct 11 16:22:49 2024
...
class Grid:
    ...
    def __init__(self, radiuss, tc, agents, beta, l, a, rho, Rv, target_locations, Negative_targets):
        ...
        ##create targets
        self.crea_targets(target_locations, Negative_targets)
        ...
        # global detected_negative_targets
        # detected_negative_targets = []  # Initialize empty list for detected negative targets
    ...
    def update(self):
        ...
        for agent in agents_list:
            agent.update(agents_list, self.rad)

        ####run clustering
        self.clustering(agents_list)
        # # Respond to detected negative targets
        # for target in detected_negative_targets:
        #     for agent in agents_list:
        #         agent.move_towards_negative_target(target)

        # ----------------------------------------------------------------------------------------------------------

    ##CREATE TARGETS using power-law distribution

    def crea_targets(self, target_locations, Negative_targets):
        self.size = 1
        ...
        for i, (x, y) in enumerate(target_locations):
            x = np.round(x % self.size, digits)
            y = np.round(y % self.size, digits)
            all_x.append(x)
            all_y.append(y)
            num = i + 1
            targets_list.append(Patch(x, y, num, 'normal'))
        for i, (x, y) in enumerate(Negative_targets):
            x = np.round(x % self.size, digits)
            y = np.round(y % self.size, digits)
            all_x.append(x)
            all_y.append(y)
            num = num + 1
            targets_list.append(Patch(x, y, num, "negative"))
        # else:
    ...
    def output(self, i):
        ...
        stringdis = str(avg_en) + "," + str(avg_dis) + "," + str(avg_eff) + "," + str(avg_size) + "," + str(
            avg_time) + "," + str(all_clusters) + "," + str(avg_simul_clusters) + "\n"
        return stringdis
...
class Patch:
    ...
    def __init__(self, x, y, num, target_type="normal"):
        # This method is called for creating a new patch
        """
        Initialize a target (Patch).

        Args:
            x: x-coordinate of the target.
            y: y-coordinate of the target.
            num: Unique identifier for the target.
            target_type: 'normal' for normal targets, 'negative' for negative targets.
        """
        self.x = x
        self.y = y
        self.num = num
        self.target_type = target_type  # "normal" or "negative"
        self.food = 1 if target_type == "normal" else -1  # Set food=0 for negative targets
...
class Agent:
    ...
    def __init__(self, num, x, y, levy, alpha, rho, Rv):
        ...
        self.negative_target = 0  ###target on which the agent is on
        ...
        self.detected_targets = []  # List to store detected target IDs
        global detected_negative_targets
        detected_negative_targets = []  ### collect detected negative targets
        self.num_det_negative_targets = 0  ### show the number of collective negative targets
        # ----------------------------------------------------------------------------------------------------------

    # def update(self, agents_list, r):  ##check if on a target or not
    #     # Reset negative target count for this time step
    #     self.num_det_negative_targets = 0  # Reset to count only new detections
    ...
    #     if num != None:
    #         target = targets_list[num - 1]
    #
    #         # Handle negative targets
    #         if target.target_type == 'negative':  # and target not in detected_negative_targets:
    #             # Add the negative target to the detected list if it's not already included
    #             self.negative_target = target.num
    #             if num not in detected_negative_targets:  # Add target ID if not already recorded
    #                 detected_negative_targets.append([self.x, self.y])
    #                 self.num_det_negative_targets += 1  ### count detected negative targets
    #
    #         if target.target_type == 'normal':  # and target.food > 0:
    #             # Collect food from the normal target
    #             self.target = target.num
    #             self.food += 1
    #             target.food = 0
    #             if num not in self.detected_targets:  # Add target ID if not already recorded
    #                 self.detected_targets.append(num)
    #
    ...
    #         ###look for others
    #         self.target = 0
    #         self.negative_target = 0  ### clear self.target and self.negative target
    ...
    #
    #         # Handle negative target movement
    #         for negative_target in detected_negative_targets:
    #             self.move_towards_negative_target(negative_target)
    def update(self, agents_list, r):
        # Reset negative target count for this time step
        self.num_det_negative_targets = 0
        # Update search efficiency
        if self.food == 0:
            self.eff = 0
            self.numcoltar = 0
        else:
            self.eff = self.food / self.tdis * 0.001  # minstep = 0.001
            self.numcoltar = self.food
        ...
        if num is not None:
            target = targets_list[num - 1]
            if target.target_type == 'negative':
                self.negative_target = target.num
                if num not in detected_negative_targets:
                    detected_negative_targets.append([self.x, self.y])
                    self.num_det_negative_targets += 1
            if target.target_type == 'normal':
                self.target = target.num
                self.food += 1
                target.food = 0
                if num not in self.detected_targets:
                    self.detected_targets.append(num)
                self.detected_target = (self.x, self.y)
                self.mu = 3
                self.alpha = 20
                self.rw_switch = 0
                self.tw_switch = 0
                self.d = 0
                return  # No movement this tick if on a target

        # If not on a target, reset target-related attributes
        self.target = 0
        self.negative_target = 0
        if self.detected_target:
            distance_to_target = st.distance.euclidean(self.detected_target, (self.x, self.y))
            if distance_to_target > self.Rv:
                self.mu = self.initial_mu
                self.alpha = self.initial_alpha
                self.detected_target = None

        # Compute cumulative velocity vector
        dx_total = 0
        dy_total = 0
        move = False

        # Step 1: Handle normal movement (RW or TW)
        if self.tw_switch == 1:
            newtarget = self.check_forothers(agents_list, r)
            if newtarget is not None:
                if self.d > 0:
                    move = True
                    dx, dy = self._compute_step_vector()
                    dx_total += dx
                    dy_total += dy
                else:
                    self.tw_switch = 0
        ...
        # Step 2: Add repulsive effect from negative targets
        for negative_target in detected_negative_targets:
            dx_rep, dy_rep = self._compute_repulsive_vector(negative_target)
            dx_total += dx_rep
            dy_total += dy_rep
            move = True

        # Step 3: Map to grid point based on cumulative vector direction
        if move and (dx_total != 0 or dy_total != 0):
            # Define possible grid points (8 neighbors at distance minstep = 0.001)
            minstep = 0.001
            grid_points = [
                (minstep, 0), (-minstep, 0), (0, minstep), (0, -minstep),  # Orthogonal
                (minstep, minstep), (minstep, -minstep), (-minstep, minstep), (-minstep, -minstep)  # Diagonals
            ]
            # Scale diagonal moves to ensure Euclidean distance is minstep
            grid_points = [(dx / np.sqrt(dx ** 2 + dy ** 2) * minstep if dx ** 2 + dy ** 2 > 0 else dx,
                            dy / np.sqrt(dx ** 2 + dy ** 2) * minstep if dx ** 2 + dy ** 2 > 0 else dy)
                           for dx, dy in grid_points]

            # Find the grid point closest to the cumulative vector direction
            best_dx, best_dy = 0, 0
            min_angle = float('inf')
            desired_angle = math.atan2(dy_total, dx_total)
            for dx, dy in grid_points:
                if dx == 0 and dy == 0:
                    continue
                grid_angle = math.atan2(dy, dx)
                angle_diff = abs((desired_angle - grid_angle + np.pi) % (2 * np.pi) - np.pi)
                if angle_diff < min_angle:
                    min_angle = angle_diff
                    best_dx, best_dy = dx, dy

            # Update position to the selected grid point
            self.x = (self.x + best_dx) % 1
            self.y = (self.y + best_dy) % 1
            self.tdis += minstep  # Update total distance traveled

            # Update remaining distance for RW/TW
            if self.d > 0:
                self.d -= minstep
                if self.d <= 0:
                    self.rw_switch = 0
            ...
            # Handle TW step length tracking
            if self.tw_switch == 1:
                if not self.tw_collecting:
                    self.tw_collecting = True
                    self.tw_step_length = 0
                self.tw_step_length += minstep
            else:
                if self.tw_collecting:
                    self.tw_collecting = False
                    self.tw_step_lengths.append(self.tw_step_length)
                    self.tw_step_length = 0

    def _compute_step_vector(self):
        r = min(self.d, 0.001)  # minstep = 0.001
        theta = self.theta * (math.pi / 180)
        dx = r * np.cos(theta)
        dy = r * np.sin(theta)
        return dx, dy

    def _compute_repulsive_vector(self, negative_target):
        dx = negative_target[0] - self.x
        dy = negative_target[1] - self.y
        dx = dx - np.round(dx)  # Periodic boundary adjustment
        dy = dy - np.round(dy)
        distance = np.sqrt(dx ** 2 + dy ** 2)
        if distance > 0:
            strength = 0.002 * np.exp(-50 * distance)
            dx = -strength * (dx / distance)
            dy = -strength * (dy / distance)
            return dx, dy
        return 0, 0

    # def move_towards_negative_target(self, negative_target):
    #     dx = negative_target[0] - self.x
    #     dy = negative_target[1] - self.y
    #
    #     dx = dx - np.round(dx)
    ...
    #     distance = np.sqrt(dx ** 2 + dy ** 2)
    #     if distance > 0:
    #         step_length = 0.002 * np.exp(-50 * distance)
    #         angle = np.arctan2(dy, dx)
    #         self.x += -step_length * np.cos(angle)
    #         self.y += -step_length * np.sin(angle)
    #
    #         self.x = self.x % 1
    #         self.y = self.y % 1
    #
    #         # Add a small random perturbation step
    #         perturbation_step_length = minstep / 2  # Adjust the magnitude of the perturbation random step as needed
    #         perturbation_angle = random.uniform(0, 2 * np.pi)
    #         self.x += perturbation_step_length * np.cos(perturbation_angle)
    #         self.y += perturbation_step_length * np.sin(perturbation_angle)
    #
    #         self.x = self.x % 1
    #         self.y = self.y % 1
    #
            ...
            if math.isinf(dis) == True:
                ...
                if self.tw_switch == 1:
                    ...
                    if self.d > dis:
                        ...
                        self.theta = self.tw_angle(self.x, self.y, new_coord[0],
                                                   new_coord[1])  ###update angle for walk
...
# In[27]:
def main_video(params, fig, ax, tick_speed, video_filename, target_locations, Negative_targets):
    ...
    grid = Grid(radiuss, tc, mono, beta, l, a, rho, Rv, target_locations, Negative_targets)
    i = 1
        ...
        while pf < 30:
            ...
            if (i % tick_speed) == 0:
                ...
                for k in agents_list:
                    ...
                    #                 print(k, theta)
                    if k.negative_target != 0:
                        ag_color.append('orange')  # Agent detecting a negative target
                        num_NT = num_NT + 1
                    elif k.target != 0:
                    ...
                    if len(detected_negative_targets) != 0:

                        # Calculate cumulative vector for move_towards_negative_target
                        cumulative_dx = 0
                        cumulative_dy = 0
                        for target in detected_negative_targets:
                            dx = target[0] - k.x
                            dy = target[1] - k.y
                            dx -= np.round(dx)  # Adjust for periodic boundaries
                            dy -= np.round(dy)
                            distance = np.sqrt(dx ** 2 + dy ** 2)
                            if distance > 0:
                                step_length = 0.002 * np.exp(-50 * distance)
                                cumulative_dx += -step_length * (dx / distance)
                                cumulative_dy += -step_length * (dy / distance)

                        # Visualize cumulative vector as an arrow
                        ax.arrow(
                            k.x,
                            k.y,
                            cumulative_dx,
                ...
                ax.scatter(tr_x, tr_y, c='green', s=6)
                ax.scatter(ntx, nty, c='red', s=6)
                if len(detected_negative_targets) != 0:
                    dtx = [];
                    dty = []
                    for mm in range(len(detected_negative_targets)):
                        loc = detected_negative_targets[mm]
                        dtx.append(loc[0])
                        dty.append(loc[1])
                    ax.scatter(dtx, dty, c='k', s=12)
                print(len(detected_negative_targets))
                ...
                plt.pause(0.01)
                plt.title(('percent_found:' + str(pf) + ';tick:' + str(i) + ';avg_efficiency:' + str(
                    np.format_float_positional(np.mean(ag_eff), precision=3)) + ';num_negative_tar:' + str(
                    len(detected_negative_targets))), fontsize=16)
                fig.canvas.draw()
    ...
    string2 = str(i) + "," + str(tc) + "," + str(mono) + "," + str(beta) + "," + str(radiuss) + "," + str(
        l) + "," + str(rho) + "," + string
    return string2


def main_timeana(params, tick_speed, hdf5_filename, case_id, target_locations, Negative_targets):
    radiuss = params[3]
    tc = params[0]
    mono = params[1]
    beta = params[2]
    ...
    all_detected_targets = []  # New list to store detected targets for all agents
    num_negative_targets = []  # New list for total negative targets per time step
    negative_target_counts = []  # New list for negative targets per agent per time step
    pf = 0
    ...
    while pf < 30:
        ...
        if (i % tick_speed) == 0:
            ...
            ag_mu = []
            tw_step = []
            target_counts = []  # To store detected targets for this time step
            neg_target_counts = []  # New list for negative targets per agent
            ...
            for k in agents_list:
                ...
                target_counts.append(len(k.detected_targets))
                neg_target_counts.append(k.num_det_negative_targets)  # Collect negative target count

            avg_efficiency = np.mean(ag_eff)
            avg_food = np.sum(ag_tar)
            num_neg_targets = len(detected_negative_targets)  # Total negative targets detected
            ...
            all_detected_targets.append(target_counts)  # Append to the main list
            num_negative_targets.append(num_neg_targets)  # Store total negative targets
            negative_target_counts.append(neg_target_counts)  # Store per-agent negative targets
        i = i + 1
    ...
    with h5py.File(hdf5_filename, 'a') as hdf5_file:
        ...
        case_group.create_dataset('average_efficiencies', data=np.array(avg_efficiencies), compression="gzip",
                                  compression_opts=9)
        case_group.create_dataset('TW_step_lengths', data=np.array(all_tw_step_lengths), compression="gzip",
                                  compression_opts=9)
        case_group.create_dataset('NumNegTar', data=np.array(detected_negative_targets), compression="gzip",
                                  compression_opts=9)

        # Save detected target counts
        case_group.create_dataset('target_counts', data=np.array(all_detected_targets), compression="gzip",
                                  compression_opts=9)
        case_group.create_dataset('num_negative_targets', data=np.array(num_negative_targets), compression="gzip",
                                  compression_opts=9)
        case_group.create_dataset('negative_target_counts', data=np.array(negative_target_counts), compression="gzip",
                                  compression_opts=9)
